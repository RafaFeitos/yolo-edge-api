...
def parse_args():
    ...
    p.add_argument("--infer-every", type=int, default=3, help="Executa YOLO a cada N frames (padrão: 3)")
    p.add_argument("--infer-size", type=int, default=320, help="Resolução de inferência em px (padrão: 320)")
    p.add_argument("--output", type=str, default=None, help="Salva o stream anotado em arquivo .avi (opcional)")
    p.add_argument("--no-display", action="store_true", help="Desativa cv2.imshow() (modo headless/SSH)")
    return p.parse_args()
