...
class TestPreprocessorOutput:
    ...
    def test_output_dtype_float32_when_normalized(self):
        """Com normalização, dtype deve ser float32 e limitado a 1.0."""
...
class TestPreprocessorConfigs:
    def test_config_low_light_applies_clahe(self):
        """Configuração low_light deve processar mantendo 3 canais de cor (RGB)."""
    ...
    def test_config_default_no_filter(self):
        """Configuração padrão não deve aplicar filtros ou CLAHE por padrão."""
...
class TestNonUniformScale:
    def test_adjust_boxes_without_letterbox_uses_separate_axis_scales(self):
        """Sem letterbox, x e y devem ser corrigidos com escalas diferentes quando a imagem de entrada não é quadrada."""
